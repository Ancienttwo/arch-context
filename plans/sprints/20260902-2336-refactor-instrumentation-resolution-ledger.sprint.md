...
| 5 | [ ] | rf3-recommendation-v3-ledger-recording | contract | `bun test packages/core/recommendation-engine packages/local-runtime/local-store-sqlite packages/local-runtime/runtime-daemon` passes with: `refactor_scan` in `ArchitectureEventSource`; `refactorRecord` RPC in dispatch table; second `record` at same HEAD returns `duplicate-active-fingerprint`; `regressesFrom` link on re-detected fingerprint; `ledger migrate --recommendation-v3` then `ledger rebuild` digest equals pre-migration rebuild digest; `recommendations resolve` without `--evidence-digest` on non-practice category returns `AC_REFACTOR_EVIDENCE_REQUIRED` | (pending) |
