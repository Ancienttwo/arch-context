...
> **Status**: Executing — FG4 In Progress
...
| FG4 | 客户控制 Organization Runner | 21 | 8 | 27 / 29 |
...
| 2026-06-21 | FG4 | Self-hosted Runner readback blocked | `bun run readback:fg4:self-hosted` records that repository self-hosted runner query returned `totalCount: 0`; local runner registration was not attempted without explicit authorization, so FG4-EG2 remains open | Codex |
| 2026-06-21 | FG4 | Public fork adversarial readback blocked | `bun run readback:fg4:public-fork` records that `Ancienttwo/arch-context` is public/forkable, but authenticated user `Ancienttwo` owns the parent repo and has no alternate org namespace; no fork was created, so FG4-EG5 remains open | Codex |
