...
# 或
archctx mcp install --host cursor
# 或输出手工配置
archctx mcp config --format json

# 验证
archctx doctor
archctx review --non-interactive
```

要求：

- `archctx install` 安装或注册同版本 `archctxd`；
- MCP Host 启动的是 `archctx mcp serve`，由其经本地 RPC 调 `archctxd`；
