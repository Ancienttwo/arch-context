...
- **Technical**: TypeScript + Node.js 24 LTS + Bun workspaces；CodeGraph 为产品硬依赖，仅经 `CodeFacts` Adapter 软耦合、精确锁版本、禁止读取其内部 DB；本地 SQLite（`node:sqlite` + WAL + FTS5），不用 PGlite；架构事实源是 Git 中 `.archcontext/` 的结构化 YAML，Markdown/图表为生成投影；长生命周期 `archctxd` daemon，CLI/MCP 为薄适配层共享 Core；MCP 默认仅暴露 5 个 `archcontext_*` 工具（`prepare_task` / `checkpoint` / `plan_update` / `apply_update` / `complete_task`）。
