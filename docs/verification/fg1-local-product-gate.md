...
- Commit SHAs:
  ...
  - `05c555f8a7d561a5e47a9e5d11bf462e9b96c1d6` — FG1-EG6 installed-bin hosted IPC matrix readback refresh
...
- Hosted Verify run `27967560199` completed Governance Verify plus all six matrix jobs after `bun install --frozen-lockfile`, `bun run verify`, and installed-bin IPC readback, then uploaded one `platform-ipc-permission-readback.json` artifact per OS/Node pair.
...
bun run verify
gh run watch 27967560199 --repo Ancienttwo/arch-context --exit-status
gh run download 27967560199 --repo Ancienttwo/arch-context --dir /tmp/archctx-27967560199-artifacts
```
