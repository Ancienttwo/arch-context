...
The FG3 connection reader and in-memory credential stores used in tests/readbacks are scaffold fixtures, not production authentication. This leaves the decision above as the target; real cross-process credential and server-side device-registration evidence is required before cloud delivery (#162).
