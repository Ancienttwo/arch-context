...
This runbook is the current individual-user install path. The pinned reproduction below targets `archctx@0.5.3`, the current published release; `0.5.1` was published with a broken release manifest and must not be installed, and `0.5.0` was never published.
...
- Required release artifact: public npm package `archctx@0.5.3`, published to `latest` on 2026-09-04.
...
- Do not treat this runbook as approved for a published package version until `docs/verification/archctx-0.5.3-release.json` is verified for that exact version.
...
```bash
npm install -g archctx@0.5.3
```

`archctx@0.5.3` is the pinned verified release: `docs/verification/archctx-0.5.3-release.json` records
...
- Public release artifact: `docs/verification/archctx-0.5.3-release.json`
