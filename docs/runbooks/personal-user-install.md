...
This runbook is the current individual-user install path. The pinned reproduction below targets `archctx@0.5.9`, the current published release; `0.5.1` was published with a broken release manifest and must not be installed, and `0.5.0` was never published.
...
```bash
npm install -g archctx@0.5.9
```

`archctx@0.5.9` is the pinned verified release: `docs/verification/archctx-0.5.9-release.json` records
...
- Public release artifact: `docs/verification/archctx-0.5.9-release.json`
