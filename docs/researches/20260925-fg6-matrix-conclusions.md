...
At the original FG6 closeout, Verify had no independent Governance dispatch. The subsequently approved entrypoint is now implemented and hosted-verified: manual run `36122160055` / source `405d6ca` passed the canonical full Governance command with all 23 evidence inspections and no matrix execution. Procedure and limits: `docs/runbooks/governance-revalidation.md`; exact result: `docs/verification/20260925-governance-revalidation.json`. The original failed workflow remains historical; a later independent success does not rewrite it. Final record-only commits are not represented as having been executed by either run. Draft PR #223 and umbrella issues #162/#164/#171 remain open. No AcceptanceReceipt, merge, release or deployment is claimed.
