...
The repo-harness/v1 documentation projection remains pending. Its current drift reports 38 files and a `human-action-required` major change (`node-added`, `node-removed`); adoption reports no candidates, so adoption cannot authorize the rename. YAML-mode ChangeSet committed no architecture-ledger event, and the projection's accepted-change contract requires a real `eventId`. Neither the ChangeSet journal ID nor an invented architecture event is substituted. A typed acceptance receipt and installed harness proof cannot be claimed until a real accepted-change event is bound and the projection reaches a canonical fixed point. The existing isolated harness candidate is still unreleased.
