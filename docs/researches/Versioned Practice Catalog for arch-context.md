...
The current public repository state also makes a **release and provenance clean-up** necessary before this work ships. The repo is currently **public**, the default branch is **`main`**, the root `package.json` still reports **version `0.1.0`**, and the GitHub tags/releases page shows **no releases**. That does not line up with the `0.1.3` reference in the review context you supplied, so the public state should be treated as an **unreconciled version mismatch** until the maintainers publish tags or update the manifest. citeturn9view1turn31view1turn32view3turn31view0
...
## Current Repo State and Relevant Code Paths

The public repository currently presents itself as **`Ancienttwo/arch-context`**, marked **Public** on GitHub, with **`main`** as the default branch. The branches page shows `main` updated on **20 June 2026** and at least one active staging branch updated on **22 June 2026**. The tags/releases page shows **no releases**. At the root, `package.json` declares `"name": "archcontext"`, `"version": "0.1.0"`, and `"private": true`. Taken together, that means the **public manifest state is `0.1.0` with no public release tags**, so any `0.1.3` reference should be treated as external context rather than something proven by the public repo. citeturn9view1turn31view1turn31view0turn32view3
