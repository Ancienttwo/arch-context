...
The DE1/DE3 readback source inputs include the extracted server and tests. Evaluating their exact source predicates before/after extraction identified three relocation regressions; the updated inputs restore equality with baseline `38159512`. Three pre-existing predicates remain false (`DE1.indexedBacklinks`, `DE3.requiredDomainsFailClosed`, `DE3.explicitAuthorityBinding`). This is source-predicate parity evidence, not a passing full DE1/DE3 record run; the historical script drift remains in #171's script cleanup scope.
