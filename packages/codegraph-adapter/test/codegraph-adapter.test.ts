...
import { MultiRepoCodeGraphAdapter, MockCodeGraphProvider } from "../src/index";

describe("@archcontext/codegraph-adapter multi-repo", () => {
  test("aggregates per-repo contexts with stable repo-scoped symbol ids", async () => {
    ...
    expect(context.symbols.map((symbol) => symbol.path)).toEqual([
      "repo.web:packages/application/src/index.ts",
      "repo.api:packages/application/src/index.ts"
    ]);
