...
function renderSvg(plan: ExplorerTopologyRenderPlan): string {
  ...
  return `<div class="topology" data-topology-mode="${plan.mode}"><svg class="topology-svg" viewBox="0 0 ${plan.width} ${plan.height}" width="100%" role="img" aria-label="Bounded architecture topology"><defs><marker id="topology-arrow" markerWidth="9" markerHeight="9" refX="8" refY="4.5" orient="auto"><path d="M0,0 L9,4.5 L0,9 Z"/></marker></defs><g class="topology-bands">${bands}</g><g class="topology-edges">${edges}</g><g class="topology-nodes">${nodes}</g>${empty}</svg></div>`;
}
