...
export function renderExplorerHtml(projection: ExplorerProjectionV2, options: RenderExplorerHtmlOptions = {}): string {
  ...
  const breadcrumb = projection.breadcrumbs.length > 0
    ? `<nav class="breadcrumb" aria-label="Breadcrumb">${projection.breadcrumbs.map((item, index) => renderBreadcrumb(item, byOccurrence, index === projection.breadcrumbs.length - 1)).join(`<span aria-hidden="true">/</span>`)}</nav>`
...
function renderBreadcrumb(item: ExplorerProjectionV2["breadcrumbs"][number], byOccurrence: Map<string, ExplorerOccurrenceV2>, current: boolean): string {
  if (current) return `<span aria-current="page">${escapeHtml(item.label)}</span>`;
  ...
  return `<button type="button" data-breadcrumb-level="context">${escapeHtml(item.label)}</button>`;
}
