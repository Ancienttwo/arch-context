...
export function renderExplorerHtml(projection: ExplorerProjectionV2, options: RenderExplorerHtmlOptions = {}): string {
  ...
  const breadcrumb = projection.breadcrumbs.length > 0
    ? `<nav class="breadcrumb" aria-label="Breadcrumb">${projection.breadcrumbs.map((item) => `<span>${escapeHtml(item.label)}</span>`).join(`<span aria-hidden="true">/</span>`)}</nav>`
    ...
    <div class="layout">
      ...
      <section class="content">
        <section class="card" aria-labelledby="map-heading">
          <div class="card-head"><div><span class="eyebrow">${escapeHtml(projection.view.question)}</span><h1 id="map-heading">${escapeHtml(projection.view.title)}</h1></div><code>${escapeHtml(projection.semanticLevel)}</code></div>
          ${renderMap(subjects, projection.relations, byOccurrence)}
        </section>
...
function renderMap(subjects: ExplorerSubjectOccurrenceV2[], relations: ExplorerRelationOccurrenceV2[], byOccurrence: Map<string, ExplorerOccurrenceV2>): string {
  if (subjects.length === 0) return empty("This semantic level contains derived groups only. Expand a group or switch to context.");
  const cards = subjects.map((subject) => {
    const subjectId = subject.subjectRefs.find((ref) => ref.kind === "architecture-entity")?.id ?? subject.subjectRefs[0]?.id ?? "";
    const incoming = relations.filter((relation) => relation.targetOccurrenceId === subject.occurrenceId).length;
    const outgoing = relations.filter((relation) => relation.sourceOccurrenceId === subject.occurrenceId).length;
    const pressure = subject.pressure.evaluated ? `${subject.pressure.level} ${subject.pressure.score}` : "not evaluated";
    return `<button type="button" class="map-node status-${subject.verificationStatus.toLowerCase()}" data-focus="${escapeHtml(subjectId)}"><strong>${escapeHtml(subject.name)}</strong><code>${escapeHtml(subject.kind)}</code><span>${escapeHtml(subject.verificationStatus)} · ${escapeHtml(subject.authorityState)}</span><small>${incoming} in · ${outgoing} out · pressure ${escapeHtml(pressure)}</small></button>`;
  }).join("");
  return `<div class="map" role="list" aria-label="Bounded architecture subjects">${cards}</div>`;
}
...
function runtimeScript(eventUrl: string, projectionDigest: string, viewDefinitionDigest: string): string {
  ...
  function go(key,value,multi){var u=new URL(window.location.href);if(multi){u.searchParams.append(key,value);}else{u.searchParams.set(key,value);}window.location.href=u.toString();}
  document.querySelectorAll("[data-view]").forEach(function(el){el.addEventListener("click",function(){go("view",el.getAttribute("data-view"));});});
  document.querySelectorAll("[data-level]").forEach(function(el){el.addEventListener("click",function(){go("level",el.getAttribute("data-level"));});});
  document.querySelectorAll("[data-focus]").forEach(function(el){el.addEventListener("click",function(){go("focus",el.getAttribute("data-focus"));});});
  document.querySelectorAll("[data-expand]").forEach(function(el){el.addEventListener("click",function(){go("expand",el.getAttribute("data-expand"),true);});});
  ...
  var current=${JSON.stringify(projectionDigest)};var currentView=${JSON.stringify(viewDefinitionDigest)};var token=new URL(window.location.href).searchParams.get("token");if(token&&window.EventSource){var source=new EventSource(${JSON.stringify(eventUrl)}+"?token="+encodeURIComponent(token));source.addEventListener("projection-invalidated",function(event){try{var data=JSON.parse(event.data);if(data.viewDefinitionDigest===currentView&&data.projectionDigest&&data.projectionDigest!==current)window.location.reload();}catch(_){source.close();}});}
})();`;
}
