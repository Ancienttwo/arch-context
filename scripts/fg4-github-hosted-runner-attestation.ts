...
export async function createFg4GithubHostedRunnerAttestation(input: { env: NodeJS.ProcessEnv; outputPath: string }) {
  ...
  const readbackId = input.env.FG4_READBACK_ID ?? "fg4-eg1-github-hosted";
  const safeReadbackId = readbackId.replace(/[^A-Za-z0-9_]+/g, "_");
  const artifactSchemaVersion = input.env.FG4_ATTESTATION_SCHEMA_VERSION ?? "archcontext.fg4-github-hosted-runner-attestation/v1";
