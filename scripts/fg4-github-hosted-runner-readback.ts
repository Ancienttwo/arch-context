...
if (import.meta.main) {
  ...
  if (command === "run") {
    ...
    console.error("[fg4-github-hosted-runner-readback] usage: run|inspect [--env-file path] [--packet path] [--out path] [--json]");
    process.exit(2);
  }
}
...
export async function buildFg4GithubHostedRunnerReadbackConfig(env: NodeJS.ProcessEnv = process.env, args: string[] = []) {
  ...
  return {
    ...
    branchName: readFlag(args, "--branch") ?? `codex/fg4-eg1-github-hosted-readback-${Date.now().toString(36)}`,
...
export async function runFg4GithubHostedRunnerReadback(config: Awaited<ReturnType<typeof buildFg4GithubHostedRunnerReadbackConfig>>) {
  ...
  const workflowLocalPath = resolve(config.root, WORKFLOW_PATH);
  ...
  try {
    ...
    pullRequestUrl = createPullRequest({
      ...
      title: "FG4 EG1 GitHub-hosted Organization Runner readback",
      body: "Temporary PR created by ArchContext FG4-EG1 readback. It is closed and deleted after evidence capture."
    });
    ...
    if (stringValue(readRecord(artifact.workflow).runnerOs) !== "Linux") failures.push("workflow artifact must come from GitHub-hosted Linux runner");
    ...
    const result = {
      schemaVersion: "archcontext.fg4-github-hosted-runner-readback/v1",
      environment: "staging",
...
export function inspectFg4GithubHostedRunnerReadback(recording: unknown) {
  ...
  if (record.schemaVersion !== "archcontext.fg4-github-hosted-runner-readback/v1") failures.push("schemaVersion mismatch");
  ...
  if (workflow.name !== WORKFLOW_NAME) failures.push("workflow name mismatch");
  ...
  if (artifact.runnerOs !== "Linux") failures.push("artifact runnerOs must be Linux");
...
function stagingWorkflowYaml(input: { installationId: number; repositoryId: number; runtimeArtifactUrl: string; runtimeArtifactDigest: string }): string {
...
jobs:
  organization-runner:
    name: GitHub-hosted Organization Runner
    ...
    steps:
      ...
      - name: Compute Challenge metadata
        ...
        run: |
          ...
          echo "FG4_EG1_WORKFLOW_REF=\${{ github.repository }}/.github/workflows/${WORKFLOW_FILENAME}@refs/heads/\${{ github.head_ref }}" >> "$GITHUB_ENV"
      ...
      - name: Run local review-action preflight
        ...
        with:
          ...
          runtime-artifact-digest: \${{ env.FG4_EG1_RUNTIME_ARTIFACT_DIGEST }}
      ...
      - name: Create Organization Attestation
        run: bun scripts/fg4-github-hosted-runner-attestation.ts create --out "$RUNNER_TEMP/fg4-eg1-organization-attestation.json"

      - name: Upload metadata-only Organization Attestation
        uses: actions/upload-artifact@v4
        with:
          name: ${ARTIFACT_NAME}
          path: \${{ runner.temp }}/fg4-eg1-organization-attestation.json
          if-no-files-found: error
`;
}

async function waitForWorkflowRun(input: { repository: string; branchName: string; headSha: string; timeoutMs: number; pollMs: number }): Promise<WorkflowRun> {
  ...
  while (Date.now() < deadline) {
    ...
    latest = runs.find((run) => run.headSha === input.headSha && run.event === "pull_request") ?? runs[0];
...
function downloadAttestationArtifact(input: { repository: string; runId: number; outputDir: string }): string {
  execFileSync("gh", [
  ...
  const artifactPath = join(input.outputDir, "fg4-eg1-organization-attestation.json");
