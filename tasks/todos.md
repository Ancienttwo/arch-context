...
| 收割 `wip/contracts-npm-scope`（contracts 无 scope 包名 + explorer-ui HTML 迁移，已备份 `origin/wip/contracts-npm-scope`） | 20260705 凌晨为 native audit 让路而 park；audit 分支工作区仍有未提交修改，避免同时开两摊 WIP | `archctx-contracts` 未发布（npm E404），ModelContext 死人开关（`MODELCONTEXT_REQUIRE_ARCHCONTEXT_CONTRACTS`）保持关闭；daemon explorer `/` 仍回 JSON 而非 HTML 页面 | `audit/native-pending-run` 落地 main 后分两刀：先单独落 npm 包名改动（`scripts/publish-archcontext-contracts.mjs` + notes/readback，与 explorer 迁移零耦合）；再 rebase explorer 迁移，重新决定渲染器归属（`core/explorer-ui` vs local-runtime 内部模块，boundary 规则见 `scripts/package-boundary-audit.mjs:122`）后走 PR |
