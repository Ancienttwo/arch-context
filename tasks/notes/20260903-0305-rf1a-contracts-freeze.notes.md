...
- **`RecommendationV3` is a discriminated union on `category`.** `RecommendationV3Base & RecommendationV3CategoryPayloadV1` makes a mismatched category/payload pair unrepresentable in typed code, while `recommendationV3InvariantIssues` still shape-guards the payload at runtime (required-key check, issues returned, never a throw) because v3 records also arrive from storage and RPC where the type is not enforced. Downstream branches only run their payload-specific checks after the shape guard passes.
