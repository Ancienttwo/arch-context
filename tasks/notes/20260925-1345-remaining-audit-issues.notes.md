...
## #225 restricted writers and committed model migration

Current verified state supersedes the historical controlled stop above. `update_adr_references` applies ADR-0021's `appliesTo` scalar edit in the same daemon ChangeSet as the capability ID and graph references. Root AGENTS/CLAUDE marker regions were rendered by the daemon from explicit current-model targets; human content is byte-preserved. 179 focused tests / 945 assertions, typecheck and package boundaries pass. Current docs projection reports 38 drift files and a `human-action-required` node rename; no accepted architecture event exists in YAML mode. Documentation projection and formal whole-plan acceptance remain pending. Evidence: `docs/researches/20260926-capability-profile-alignment.md`, `docs/verification/20260926-restricted-writer-migration.json`.
